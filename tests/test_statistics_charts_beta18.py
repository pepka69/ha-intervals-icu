...
def test_statistics_dashboard_exposes_evolution_history() -> None:
    ...
    data = {
        ...
        "wellness_sleep_history": [
            {"date": "2026-07-01", "value": 28800.0}
        ],
        "wellness_hrv_history": [
            {"date": "2026-07-01", "value": 48.0}
        ],
        "wellness_resting_hr_history": [
            {"date": "2026-07-01", "value": 52.0}
        ],
        "wellness_readiness_history": [
    ...
    assert (
        attributes["evolution"]["resting_hr"]
        == data["wellness_resting_hr_history"]
    )
    assert (
        attributes["evolution"]["readiness"]
        == data["wellness_readiness_history"]
    )
...
def test_statistics_frontend_contains_native_chart_infrastructure() -> None:
    ...
    chart_source = (ROOT / "frontend/src/chart.ts").read_text(
        encoding="utf-8"
    )
    card_source = (ROOT / "frontend/src/statistics-card.ts").read_text(
        encoding="utf-8"
    )
